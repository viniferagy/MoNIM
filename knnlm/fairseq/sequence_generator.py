    ...
    def _generate(
        ...
        **kwargs
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            # print(tokens[:, :step + 1].shape)
            # for i in range(tokens.shape[0]):
            #     print(self.tokenizer.convert_tokens_to_string(self.dictionary.string(tokens[i, :step + 1]).split()))
            lprobs, avg_attn_scores = model.forward_decoder(
                tokens[:, :step + 1], encoder_outs, temperature=self.temperature, **kwargs, lmbda=self.args.lmbda
            )
...
class EnsembleModel(torch.nn.Module):
    ...
    def _decode_one(
        ...
        if 'knn_dstore' in kwargs:
            ...
            # len x (bsz*beam_size) x dim
            queries = decoder_out[1]['last_ffn_input']
            knn_prob = dstore.get_knn_log_prob(
                        queries.permute(1,0,2)[:, -1, :].contiguous(),
                        )
            combine_probs = torch.stack([probs, knn_prob], dim=0)
            coeffs = torch.ones_like(combine_probs)
            lmbda = kwargs['lmbda']
            coeffs[0] = np.log(1 - lmbda)
            coeffs[1] = np.log(lmbda)
            probs = torch.logsumexp(combine_probs + coeffs, dim=0)
