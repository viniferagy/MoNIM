...
class TokenFeatureDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return{
            ...
            'val_id': hypo_other['val_id'] if hypo_other else None, 
            'val_count': hypo_other['val_count'] if hypo_other else None,
            'old_distance': hypo_other['old_d'] if hypo_other else None,
            'tgt': hypo_other['tgt'] if hypo_other else None,
        }
    ...
    def collater(self, samples):
        ...
        batch = {
            'feature':{
                ...
                'fert': merge('fert'),
                'val_id': merge('val_id'),
                'val_count': merge('val_count'),
                'old_distance': merge('old_distance'),
                'tgt': merge('tgt'),
            },
            'id': merge('id'),
